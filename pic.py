...
Ti = 50;
Te = 50;
lambda_d_phys = np.sqrt(eps0*K*Te/(n0*q_phys**2))
...
delta_x = lambda_d_phys/20
delta_t = 0.001/omega_p_phys
...
def interpolate_charges(n_x, x_i_N, x_e_N, mnum_i_N, mnum_e_N):
...
#    
    chg_j_i_N = mnum_i_N*((ind_x_i_N+1)-x_i_N)
    chg_jp1_i_N = mnum_i_N*(x_i_N-ind_x_i_N)
    chg_j_e_N = -mnum_e_N*((ind_x_e_N+1)-x_e_N)
    chg_jp1_e_N = -mnum_e_N*(x_e_N-ind_x_e_N)
...
def solve_efield_from_potential_periodic(phi_N):
    """Given potential field, solves for e-field in 1D"""
    
    # Perform calculation
    E_N = -(np.roll(phi_N, -1) - np.roll(phi_N, 1))/2
    
    # Return E-field    
    return(E_N)


def integrate_motion(n_x, delta_x, delta_t, E_N, x_i_N, x_e_N, v_i_N, v_e_N, mnum_i_N, mnum_e_N):
...
def calculate_energy(x_i_old_N, x_e_old_N, v_i_old_N, v_e_old_N, v_i_new_N, v_e_new_N, mnum_i_N, mnum_e_N, chg_N, phi_N):
    ...
    v_i_t_squared = v_i_old_N*v_i_new_N
    v_e_t_squared = v_e_old_N*v_e_new_N
    
    # Calculate kinetic energy for each species
    KE_i_N = 1/2*mi_me_ratio*np.inner(mnum_i_N, v_i_t_squared)
    KE_e_N = 1/2*np.inner(mnum_e_N, v_e_t_squared)
    
    # Calculate electric potential energy
    PE_N = np.inner(chg_N, phi_N)
    
    # Calculate total energy 
    E_tot_N = KE_i_N + KE_e_N + PE_N
    
    # Print stuff
#    print("Ion kinetic: " + str(KE_i_N))
#    print("Electron kinetic: " + str(KE_e_N))
#    print("Electric potential: " + str(PE_N))
...
#    x_e_N = np.arange(n_x/(2*NP)+margin,n_x-margin,n_x/NP)
    ...
    for t_N in np.arange(n_tsteps):
        ...
        x_i_new_N, x_e_new_N, v_i_new_N, v_e_new_N = integrate_motion(n_x, delta_x, delta_t, E_N, x_i_N, x_e_N, v_i_N, v_e_N, mnum_i_N, mnum_e_N)
        
        # Calculate total energy
        E_old = calculate_energy(x_i_N, x_e_N, v_i_N, v_e_N, v_i_new_N, v_e_new_N, mnum_i_N, mnum_e_N, chg_N, phi_N)
        
        # Print to console
        print("Total system energy = " + str(E_old))
        ...
        plt.figure()
        plt.plot(np.arange(0,n_x,1), chg_N)
